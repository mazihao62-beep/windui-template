--[[
    WindUI 通用脚本模板 v3.2（开箱即用版）
    
    v3.2 三大致命Bug修复（同步主线v12.2）:
    B1: 主题色不更新 - ThemeColors硬编码名不匹配 → 动态getThemePrimaryColor()四层保障
    B2: 粒子超出UI - 旧边界在Sidebar内 → X:0.35~0.78 Y:0.25~0.80
    B3: 初始位置超出 - 创建时随机范围与边界不一致 → 统一用边界常量
...
local function tagTrack(instance)
    if not instance then return nil end; pcall(function() instance:SetAttribute(TAG_NAME, true) end); return instance
end

task.spawn(function() task.wait(0.1); local count = 0; pcall(function() for _, inst in ipairs(CoreGui:GetDescendants()) do local s,a = pcall(function() return inst:GetAttribute(TAG_NAME) end); if s and a then pcall(function() inst:Destroy() end); count = count + 1 end end end); if count > 0 then print("[清理] 已清除 " .. count .. " 个旧实例") end end)

-- ============================================================
--  第3部分：设置项
-- ============================================================
local Settings = {
    Particles = true, CurrentTheme = "Dark", ParticleColor = Color3.fromRGB(100, 180, 255),
...
--  安全范围: X 0.35~0.78, Y 0.25~0.80
-- ============================================================
local PARTICLE_MIN_X = 0.35; local PARTICLE_MAX_X = 0.78
local PARTICLE_MIN_Y = 0.25; local PARTICLE_MAX_Y = 0.80

-- ============================================================
--  第5部分：主题色提取系统（v3.2重写）
--  四层保障:
--    1. 从WindUI:GetThemes()动态获取主题定义的颜色字段
--    2. case-insensitive匹配静态ThemeColors表
--    3. 同义词匹配
--    4. 名字hash生成颜色
-- ============================================================
local ThemeColors = {
...
local function nameToColor(n) local h=0;for i=1,#n do h=h+string.byte(n,i)end;local s=math.sin(h*137.5)*0.5+0.5;local s2=math.sin(h*73.1+50)*0.5+0.5;local s3=math.sin(h*41.7+200)*0.5+0.5;return Color3.fromRGB(math.floor(80+s*175),math.floor(80+s2*175),math.floor(120+s3*135)) end

local function getThemePrimaryColor(name)
    if not name then return Color3.fromRGB(100,180,255) end; local l=name:lower()
    local themes=nil;pcall(function()themes=WindUI:GetThemes()end)
    if themes and themes[name] then local d=themes[name];local c=nil;pcall(function()if type(d)=="table" then c=d.Primary or d.Accent or d.Color or d.Main or d.PrimaryColor end end);if c then return c end end
    local m=ThemeColors[l];if m then return m end
    if l:find("dark")or l:find("night")then return Color3.fromRGB(100,180,255)end;if l:find("light")or l:find("white")then return Color3.fromRGB(80,140,200)end;if l:find("rose")or l:find("pink")then return Color3.fromRGB(255,120,160)end;if l:find("plant")or l:find("green")or l:find("forest")or l:find("mint")then return Color3.fromRGB(100,200,120)end;if l:find("ocean")or l:find("blue")or l:find("sky")then return Color3.fromRGB(80,180,230)end;if l:find("sunset")or l:find("orange")or l:find("coral")then return Color3.fromRGB(255,150,80)end;if l:find("midnight")or l:find("purple")or l:find("lavender")then return Color3.fromRGB(120,100,220)end;if l:find("blood")or l:find("red")then return Color3.fromRGB(220,80,80)end;if l:find("lemon")or l:find("yellow")then return Color3.fromRGB(220,200,80)end
...
--  第6部分：内部变量
-- ============================================================
local WindowRef=nil;local FloatingButtonGui=nil;local ParticleGui=nil;local ParticleRunning=false;local PopupConfirmed=false
local Controls={};local Keybinds={};local TabElements={};local ConfigName="default";local Particles={}
...
--  第7部分：粒子背景系统 v3.2
-- ============================================================
local function getParticleColor() return Settings.ParticleColor or Color3.fromRGB(100,180,255) end
...
local function createParticles()
    ...
    pcall(function()
        ParticleGui=tagTrack(Instance.new("ScreenGui"));ParticleGui.Name="Template_Particles";ParticleGui.ResetOnSpawn=false;ParticleGui.DisplayOrder=-999;ParticleGui.IgnoreGuiInset=true;ParticleGui.Parent=CoreGui
        local rx=PARTICLE_MAX_X-PARTICLE_MIN_X;local ry=PARTICLE_MAX_Y-PARTICLE_MIN_Y
        for i=1,50 do
            local dot=tagTrack(Instance.new("Frame"));local s=math.random(4,8);dot.Size=UDim2.new(0,s,0,s)
            dot.Position=UDim2.new(PARTICLE_MIN_X+math.random()*rx,0,PARTICLE_MIN_Y+math.random()*ry,0)
            dot.BackgroundColor3=getParticleColor();dot.BackgroundTransparency=0.4+math.random()*0.4;dot.BorderSizePixel=0;dot.Parent=ParticleGui
            tagTrack(Instance.new("UICorner"));dot.UICorner.CornerRadius=UDim.new(0,10)
            local a=math.random()*6.28;local sp=0.0002+math.random()*0.0006
            table.insert(Particles,{Frame=dot,Vx=math.cos(a)*sp,Vy=math.sin(a)*sp,Phase=math.random()*6.28,SizeBase=s,MinBoundX=PARTICLE_MIN_X,MaxBoundX=PARTICLE_MAX_X,MinBoundY=PARTICLE_MIN_Y,MaxBoundY=PARTICLE_MAX_Y})
        end
        ParticleRunning=true
        task.spawn(function()local t=0;while ParticleRunning and ParticleGui and ParticleGui.Parent do t=t+0.03;pcall(function()for _,p in ipairs(Particles)do if not p.Frame or not p.Frame.Parent then continue end;local x=p.Frame.Position.X.Scale+p.Vx;local y=p.Frame.Position.Y.Scale+p.Vy;if x>=p.MaxBoundX then x=p.MaxBoundX;p.Vx=-(math.abs(p.Vx)*0.95)+(math.random()-0.5)*0.0001 elseif x<=p.MinBoundX then x=p.MinBoundX;p.Vx=math.abs(p.Vx)*0.95+(math.random()-0.5)*0.0001 end;if y>=p.MaxBoundY then y=p.MaxBoundY;p.Vy=-(math.abs(p.Vy)*0.95)+(math.random()-0.5)*0.0001 elseif y<=p.MinBoundY then y=p.MinBoundY;p.Vy=math.abs(p.Vy)*0.95+(math.random()-0.5)*0.0001 end;p.Frame.Position=UDim2.new(x,0,y,0);p.Frame.BackgroundTransparency=0.4+math.sin(t*0.8+p.Phase)*0.25;local sz=math.max(1,p.SizeBase+math.sin(t+p.Phase)*0.8);p.Frame.Size=UDim2.new(0,sz,0,sz)end end);task.wait(0.03)end end)
    end)
end
...
if s and r then
    ...
    WindUI:Popup({Title="WindUI 脚本模板 v3.2",Icon="solar:info-square-bold",Content=[[📋 NPC透视 - 高亮显示 + 头顶标签
💾 配置保存 - 自动保存/读取设置
🎨 主题系统 - 16种内置主题一键切换
✨ 粒子背景 - 内容区边界(X:0.35~0.78, Y:0.25~0.80)
🌀 增强毛玻璃 - Acrylic + 透明叠加
🧹 脚本Tag清理 - 重启自动清除所有残留
...
⚠️ 加载后所有功能默认关闭，需手动开启]], Buttons={{Title="取消",Callback=function()end,Variant="Tertiary"},{Title="确认加载",Icon="solar:arrow-right-bold",Callback=function()PopupConfirmed=true;pcall(function()WindUI:Notify({Title="✅ 已加载",Content="⌨️ 按 RightShift 打开菜单",Duration=4,Icon="solar:bell-bold"})end);task.spawn(function()createWindow()end)end,Variant="Primary"}}})
    ...
    function createWindow()
        ...
        local ut=win:Tab({Title="UI设置",Icon="solar:monitor-bold"});ut:Paragraph({Title="⚙️ 界面设置"});Controls.WindowKeybind=ut:Keybind({Flag="WindowKeybind",Title="窗口开关快捷键",Value="RightShift",Callback=function(k)Keybinds.Window=k;if WindowRef then pcall(function()WindowRef:SetToggleKey(Enum.KeyCode[k])end)end end});Controls.FloatingBtnToggle=ut:Toggle({Flag="FloatingBtnToggle",Title="显示悬浮按钮",Value=IsMobile,Callback=function(v)if FloatingButtonGui then FloatingButtonGui.Enabled=v end end});ut:Divider();ut:Paragraph({Title="🌀 背景效果"});Controls.ParticlesToggle=ut:Toggle({Flag="ParticlesToggle",Title="浮动粒子背景(50个)",Value=true,Callback=function(v)Settings.Particles=v;if v then createParticles()else destroyParticles()end end});ut:Divider();ut:Paragraph({Title="✨ 窗口效果"});Controls.AcrylicToggle=ut:Toggle({Flag="AcrylicToggle",Title="毛玻璃效果",Value=true,Callback=function(v)pcall(function()WindUI:ToggleAcrylic(v)end)end});Controls.TransparencyToggle=ut:Toggle({Flag="TransparencyToggle",Title="透明背景增强毛玻璃",Value=true,Callback=function(v)if WindowRef then pcall(function()WindowRef:ToggleTransparency(v)end)end end});ut:Divider();ut:Paragraph({Title="🎨 主题系统",Desc="切换主题时粒子颜色自动适配"})
        local allThemes={};pcall(function()allThemes=WindUI:GetThemes()end);local tns={};for n,_ in pairs(allThemes)do table.insert(tns,n)end;table.sort(tns)
        ...
        local at=win:Tab({Title="关于",Icon="solar:info-square-bold"});at:Paragraph({Title="WindUI 脚本模板 v3.2",Desc="三大Bug修复: 主题色/边界/初始位置"});at:Divider();at:Paragraph({Title="👤 作者",Desc="b站英吉利超入_"});at:Divider();at:Paragraph({Title="💡 使用说明",Desc=IsMobile and"手机: 点击悬浮按钮"or"PC: 按 RightShift 打开菜单"});at:Paragraph({Title="⚠️ 提示",Desc="所有功能默认关闭，请在菜单中手动开启"});at:Paragraph({Title="🧹 清理",Desc="重启脚本自动清除所有残留 (脚本Tag系统)"})
