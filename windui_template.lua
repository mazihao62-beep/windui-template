--[[
    WindUI 通用脚本模板 v3.1（开箱即用版）
    
    v3.1 UI Bug审查修复:
    B1: 手机按钮 WindowRef:Toggle() 不存在 → VirtualInputManager 模拟ToggleKey
    B2: ThemeColors[selected] 可能为 nil → 备用蓝色
    B3: RunService 未使用 → 移除
    B4: beautifyUI 只运行一次 → 每3秒持续运行
    B5: 手机悬浮按钮拖拽越界 → 边界钳制
...
local function createParticles()
    ...
    pcall(function()
        ...
        for i=1,50 do
            local dot=tagTrack(Instance.new("Frame")); local size=math.random(4,8); dot.Size=UDim2.new(0,size,0,size)
            dot.Position=UDim2.new(0.28+math.random()*0.30,0,0.18+math.random()*0.32,0); dot.BackgroundColor3=getParticleColor(); dot.BackgroundTransparency=0.4+math.random()*0.4; dot.BorderSizePixel=0; dot.Parent=ParticleGui
        ...
        task.spawn(function() local t=0; while ParticleRunning and ParticleGui and ParticleGui.Parent do t=t+0.03; pcall(function() for _,p in ipairs(Particles) do if not p.Frame or not p.Frame.Parent then continue end; local x=p.Frame.Position.X.Scale+p.Vx; local y=p.Frame.Position.Y.Scale+p.Vy; if x>p.MaxBoundX then x=p.MaxBoundX; p.Vx=-p.Vx+(math.random()-0.5)*0.0001 elseif x<p.MinBoundX then x=p.MinBoundX; p.Vx=-p.Vx+(math.random()-0.5)*0.0001 end; if y>p.MaxBoundY then y=p.MaxBoundY; p.Vy=-p.Vy+(math.random()-0.5)*0.0001 elseif y<p.MinBoundY then y=p.MinBoundY; p.Vy=-p.Vy+(math.random()-0.5)*0.0001 end; p.Frame.Position=UDim2.new(x,0,y,0); p.Frame.BackgroundTransparency=0.4+math.sin(t*0.8+p.Phase)*0.25; local s=math.max(1,p.SizeBase+math.sin(t+p.Phase)*0.8); p.Frame.Size=UDim2.new(0,s,0,s) end end); task.wait(0.03) end end)
    end)
end
...
local function classifyNPC(humanoid, npcModel, npcName, fullPath)
    ...
    local tc=humanoid.TeamColor; if tc then if tc==BrickColor.new("Bright blue") or tc==BrickColor.new("Bright green") then return "Good" end; if tc==BrickColor.new("Bright red") or tc==BrickColor.new("Really black") then return "Bad" end end; return "Bad"
end

local function createNPCESP(nm)
    if not nm or not nm.PrimaryPart then return end; if ESP_Objects[nm] then return end
    local h=nm:FindFirstChildOfClass("Humanoid"); local hd=nm:FindFirstChild("Head"); if not h or not hd then return end
    local nt=classifyNPC(h,nm,nm.Name,nm:GetFullName()); local ig=(nt=="Good"); local c=ig and Color3.fromRGB(0,255,80) or Color3.fromRGB(255,50,50); local lb=ig and "👮 好人" or "💀 坏人"
    ...
    local fr=tagTrack(Instance.new("Frame")); fr.Size=UDim2.new(1,-10,1,-10); fr.Position=UDim2.new(0,5,0,5); fr.BackgroundColor3=Color3.fromRGB(0,0,0); fr.BackgroundTransparency=0.35; fr.BorderSizePixel=0; fr.Parent=bg
    tagTrack(Instance.new("UICorner")); fr.UICorner.CornerRadius=UDim.new(0,6)
    ...
    local hb=tagTrack(Instance.new("Frame")); hb.Size=UDim2.new(0.8,0,0,4); hb.Position=UDim2.new(0.1,0,0,46); hb.BackgroundColor3=Color3.fromRGB(60,60,60); hb.BackgroundTransparency=0.3; hb.BorderSizePixel=0; hb.Parent=fr
    local hf=tagTrack(Instance.new("Frame")); hf.Size=UDim2.new(1,0,1,0); hf.BackgroundColor3=ig and Color3.fromRGB(0,200,100) or Color3.fromRGB(200,50,50); hf.BorderSizePixel=0; hf.Parent=hb
    tagTrack(Instance.new("UICorner")); hb.UICorner.CornerRadius=UDim.new(0,2)
    ESP_Objects[nm]={Model=nm,Humanoid=h,Head=hd,Highlight=hl,Billboard=bg,Frame=fr,TypeLabel=tl,InfoLabel=il,HPBar=hf,HPBg=hb,IsGood=ig,Label=lb,Color=c}
end

local function updateAllESP()
    if not Settings.ESP_Enabled then return end
    local ch=PlayerChar; if not ch or not ch.PrimaryPart then local p=Players.LocalPlayer; if p then pcall(function() ch=p.Character end); PlayerChar=ch end end
    local mp=ch and ch.PrimaryPart and ch.PrimaryPart.Position
    for md,esp in pairs(ESP_Objects) do pcall(function() if not md or not md.Parent then if esp.Highlight then esp.Highlight:Destroy() end; if esp.Billboard then esp.Billboard:Destroy() end; ESP_Objects[md]=nil; return end; if Settings.ESP_BadOnly and esp.IsGood then esp.Highlight.Enabled=false; esp.Billboard.Enabled=false; return end; esp.Highlight.Enabled=true; esp.Billboard.Enabled=true; local dt=""; if Settings.ESP_ShowDistance and mp and esp.Head then local d=(esp.Head.Position-mp).Magnitude; if d<=Settings.ESP_MaxRange then dt=string.format("%.0fm",d) else esp.Highlight.Enabled=false; esp.Billboard.Enabled=false; return end end; local ht=""; local ch=100; local mh=100; if Settings.ESP_ShowHealth then pcall(function() ch=esp.Humanoid.Health; mh=esp.Humanoid.MaxHealth end); ht=string.format("HP: %.0f/%.0f",ch,mh); esp.HPBar.Size=UDim2.new(math.max(0,ch/mh),0,1,0) end; local ps={}; if dt~="" then table.insert(ps,dt) end; if ht~="" then table.insert(ps,ht) end; esp.InfoLabel.Text=table.concat(ps," | "); esp.HPBg.Visible=Settings.ESP_ShowHealth end) end
end
...
if s and r then
    ...
    function createWindow()
        ...
        Controls.ThemeDropdown=ut:Dropdown({Flag="ThemeDropdown", Title="选择主题", Values=tns, Value="Dark", Callback=function(sl) if sl then Settings.CurrentTheme=sl; pcall(function() WindUI:SetTheme(sl) end); Settings.ParticleColor=ThemeColors[sl] or Color3.fromRGB(100,180,255); updateParticleColor() end end})
        ...
        ct:Space(); local CM=WindowRef.ConfigManager; local AC={}; pcall(function() AC=CM:AllConfigs() end); local DV=nil; pcall(function() for _,v in ipairs(AC) do if v=="default" then DV="default"; break end end end)
        ...
        at:Paragraph({Title="WindUI 脚本模板 v3.1", Desc="UI Bug审查修复版本"})
