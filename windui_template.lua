...
if not LO then local m=Instance.new("Message")m.Text="WindUI加载失败("..mR.."次重试)";m.Parent=workspace;task.delay(4,function()m:Destroy()end);return end

local P,U=game:GetService("Players"),game:GetService("UserInputService")
local W,C=game:GetService("Workspace"),game:GetService("CoreGui")
...
local function clean()
    local c=0;pcall(function()
        for _,v in ipairs(C:GetDescendants())do local ok,a=pcall(function()return v:GetAttribute(TAG)end)
            if ok and a then pcall(function()v:Destroy()end);c=c+1 end end
        local wc=0;for _,g in ipairs(C:GetChildren())do if g:IsA("ScreenGui")then local n=g.Name
            ...
            elseif n:find("Tpl")or n:find("ESP_Particles")then pcall(function()g:Destroy()end);c=c+1 end end end end)
    if c>0 then print("[清理] "..c.." 个残留实例")end
end
clean();_G.CleanupTpl=function()clean()end
...
local function gtc(n)
    if not n then return Color3.fromRGB(80,170,255)end;local l=n:lower();local m=TC[l];if m then return m end
    ...
    if l:find("plant")or l:find("green")then return Color3.fromRGB(70,210,130)end;if l:find("ocean")or l:find("blue")then return Color3.fromRGB(60,190,240)end
    if l:find("sunset")or l:find("orange")then return Color3.fromRGB(255,160,70)end;if l:find("midnight")or l:find("purple")then return Color3.fromRGB(130,100,240)end
...
local function cp()
    ...
    if not S.Particles then return end;task.wait(0.5)
    pcall(function()
        local sg=Instance.new("ScreenGui");sg.Name="Tpl_Particles";sg.ResetOnSpawn=false;sg.DisplayOrder=999999;sg.IgnoreGuiInset=true;sg.Parent=C
        ...
        for i=1,50 do local d=Instance.new("Frame");local sz=math.random(5,10);d.Size=UDim2.new(0,sz,0,sz)
            local sx=0.2+math.random()*0.6;local sy=0.2+math.random()*0.6;d.Position=UDim2.new(sx,0,sy,0)
        ...
        task.spawn(function()local t=0;while PR and PC do t=t+0.03
            pcall(function()local curCol=S.ParticleColor;for _,p in ipairs(PS)do if p.F and p.F.Parent then
                ...
                p.F.BackgroundTransparency=0.3+math.sin(t*0.8+p.Ph)*0.4;local bs=math.max(2,p.Sz+math.sin(t+p.Ph)*1.5);p.F.Size=UDim2.new(0,bs,0,bs)end end end)
...
local function cw()
    ...
    local ok2,w=pcall(function()return WI:CreateWindow({
        Title="WindUI模板",Author="b站英吉利超入_",Icon="solar:code-bold",Size=UDim2.fromOffset(750,520),
        ToggleKey=Enum.KeyCode.RightShift,Folder="windui-template",Acrylic=true,Transparent=true,Resizable=false,
        ...
        OnOpen=function()if S.Particles then task.spawn(function()task.wait(0.5);cp()end)end end})end)
    ...
    CT.TT=ut:Toggle({Flag="TT",Title="透明背景",Value=true,Callback=function(v)pcall(function()if WN then pcall(function()WN:ToggleTransparency(v)end)end end)end})
    ...
    CT.TD=ut:Dropdown({Flag="TD",Title="选择主题",Values=tn,Value="Dark",Callback=function(sl)if sl and type(sl)=="string"then S.CurrentTheme=sl;WI:SetTheme(sl);S.ParticleColor=gtc(sl);uPC()end end})

    local st=WN:Tab({Title="信息统计",Icon="solar:chart-bold"})
    -- ⬇️ 【在此添加统计显示项】 ⬇️
    TE.Item1=st:Paragraph({Title="📊 统计项 1: 0"});TE.Item2=st:Paragraph({Title="📊 统计项 2: 0"});TE.Item3=st:Paragraph({Title="📊 统计项 3: 0"})
    ...
    at:Paragraph({Title="WindUI模板 v6.3",Desc="WindUI加载放最前面+去除多行注释"})
    ...
    print("[v6.3] WindUI模板已加载")
end

pcall(function()WI:SetTheme("Dark")end);S.ParticleColor=gtc("Dark")
WI:Popup({Title="WindUI模板 v6.3",Icon="solar:info-square-bold",
    Content="✨ 6Tab标准UI框架\n🔘 WindUI内置OpenButton\n🌀 粒子背景\n🎨 16种主题\n💾 配置保存",
    Buttons={{Title="取消",Callback=function()end,Variant="Tertiary"},
        {Title="确认加载",Icon="solar:arrow-right-bold",Callback=function()PP=true
            WI:Notify({Title="✅ 已加载",Content="按RightShift打开菜单",Duration=4,Icon="solar:bell-bold"});task.spawn(cw)end,Variant="Primary"}}})
